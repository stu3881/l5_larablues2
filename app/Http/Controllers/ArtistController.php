...
class ArtistController extends CRHBaseController
{
    ...
    //$this->debug0(__FILE__,__LINE__,__FUNCTION__);
        switch ($distinct_regular) { 
            ...
            case "distinct":
                $query = Artist::distinct()->select($field_name);
                echo("Artist::distinct()->select(".$field_name.")");
                break;
            case "regular":
                $query = Artist::where($field_name,$r_o,$v);
                echo("Artist::where(".$field_name.",". $r_o. ",".$v.")");
                break;
       }   
       return $query;
    }
