...
class UsersController extends CRHBaseController
{
    ...
    // ****************
        ...
        switch ($distinct_regular) { 
            ...
            case "distinct":
                $query = User::distinct()->select($field_name);
                echo("User::distinct()->select(".$field_name.")");
                break;
            case "regular":
                $query = User::where($field_name,$r_o,$v);
                echo("User::where(".$field_name.",". $r_o. ",".$v.")");
                break;
       }   
       return $query;
    }
    ...
    public function kloneRecord($function,$id)    {
        switch ($function) {
             case "insert":
                //$this->debugx('1111',__FILE__,__LINE__,__FUNCTION__);
                $data_record = User::where('id','=',$id)->get();
        ...
        if (!empty($what_we_are_doing)) {
            ...
            switch ($what_we_are_doing) { 
                ...
                // *****
                        ...
                        return view('users'.'.editUpdate',compact('miscThings'))
                        ->with('node_name'   ,$this->node_name)            
                        ->with('model'   ,$this->model)            
        ...
        if ($update){  
            ...
            switch ($what_we_are_doing) {
                ...
                case 'updating_report_name':
                    switch ($coming_from) {
                        ...
                        case 'select_fields':
                           ...
                           return view('users'.'.reportDefMenuEdit',compact('miscThing'))
                            ->with('id'                    ,$id)
                ...
                case "ppv_define_business_rules":
                   ...
                   return redirect()->route('users.browseEdit', 
