		...
		*/		
		...
		<!--	the buttons at the end of the line -->
			<td>	
				...
				{{ Form::open(array('url'=>'admin/'.$node_name.'/{'.$model.'}/'.'reportDefEdits'.$record->id,'method'=>'PUT')) }}
				{{ Form::hidden('logical_button_name'					,'maintain_modifiabl.e_fields') }}	
				...
				{{ Form::submit('reportDefEdits', array('class'=>'mycart-btn-row2')) }}
				{{ Form::close() }}
			</td>		
	             <td>
				...
				<td >
	             ...
	             <td>
	             	<a href="{{route($node_name.'.browseEdit',$record->id)}}" class="btn btn-warning">browseEdit</a>
	             </td>
